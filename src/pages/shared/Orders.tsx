...
import { Eye, FileText, Package, Search, Plus, Minus, Trash2 } from 'lucide-react';
...
const Orders = ({ isOrgAdmin }: OrdersProps) => {
  ...
  const { 
    ...
    queryKey: ['orders', userData?.id, userData?.user_type, isOrgAdmin],
    queryFn: async () => {
      try {
        let response;
        
        if (isOrgAdmin && userData?.org_id) {
          // Org admin - get orders for organization
          response = await orderService.getOrdersByOrganization(userData.org_id);
        } else if (!isOrgAdmin && userData?.id) {
          // Individual - get orders for self
          response = await orderService.getOrdersByUser(userData.id, 'INDIVIDUAL');
        } else {
          throw new Error('Unable to fetch orders - invalid user data');
        }
  ...
  // Update order status mutation (only available for org admin)
  const updateOrderStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['orders', userData?.id, userData?.user_type, isOrgAdmin] });
      if (selectedOrder) {
        refetchOrderDetails();
      }
    },
  ...
  // Handle update status (org admin only)
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex justify-between items-center">
        ...
        <Button
          ...
          asChild
        >
          <a href={isOrgAdmin ? "/org-admin/orders/new" : "/individual/orders/new"}>
            <Plus size={16} /> 
            Place New Order
          </a>
        </Button>
                ...
                {isLoading ? (
                  ...
                  filteredOrders.map((order: Order) => (
                    <TableRow key={order.id}>
                      ...
                      {isOrgAdmin && <TableCell>{order.user_name || 'Unknown'}</TableCell>}
